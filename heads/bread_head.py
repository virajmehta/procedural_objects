...
class BreadHead(Head):
    def __init__(self,
        ...
        self.scad = 'l = {0}; w = {1}; translate([0, {3}, {4}]) {{rotate(a=[0,{2},0]){{rotate(a=[90,0,0]){{translate([0,0, - l/2]){{linear_extrude(height=l, $fn=60) {{ translate([0, w/2.6, 0]) {{ circle(w/2); }} translate([-w/2, -w/2, 0]) {{ square(w); }} }} }} }} }} }};' # NOQA 
    ...
    def get_random_scad(self):
        ...
        return self.scad.format(length, radius, tilt, x_offset, z_offset)
